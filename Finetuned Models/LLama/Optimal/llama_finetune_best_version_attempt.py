# -*- coding: utf-8 -*-
"""LLama_finetune-best_version_attempt.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1zGpZ7TTp-RFkJHz_jZ01x_cgn8tvutRR
"""
...
"""# LLAMA Prompt (Need to modify this)"""
...
# def compute_target_frequencies(train_data):
#     """Compute frequency of each target across the training data."""
#     target_freqs = Counter()
#     for sample in train_data:
#         for target, is_active in sample["targets"].items():
#             if is_active:
#                 target_freqs[target] += 1
#     return target_freqs

# def select_examples_stratified_diverse(train_data, current_idx, target_freqs, k=3):
#     """
#     Strategy: Ensure label diversity + maximize rare target coverage.
#     """
#     by_label = {"hateful": [], "neutral": [], "supportive": []}
#     for i, sample in enumerate(train_data):
#         if i != current_idx:
#             by_label[sample["overall"]["label"]].append(i)

#     selected = []
#     covered_targets = set()

#     for label in ["hateful", "neutral", "supportive"]:
...
#             active_targets = [t for t, v in sample["targets"].items() if v]
#             rarity = sum(1.0 / (target_freqs[t] + 1) for t in active_targets if t not in covered_targets)
#             scored.append((idx, rarity))
#         scored.sort(key=lambda x: -x[1])

#         best_idx = scored[0][0]
#         selected.append(best_idx)
#         for t, v in train_data[best_idx]["targets"].items():
#             if v:
...
#     if remaining > 0:
#         all_candidates = [i for i in range(len(train_data)) if i != current_idx and i not in selected]
#         scored = []
#         for idx in all_candidates:
#             sample = train_data[idx]
#             active = [t for t, v in sample["targets"].items() if v]
#             score = sum(1.0 / (target_freqs[t] + 1) for t in active if t not in covered_targets)
#             scored.append((idx, score))
#         scored.sort(key=lambda x: -x[1])

#         for idx, _ in scored[:remaining]:
#             selected.append(idx)
#             for t, v in train_data[idx]["targets"].items():
#                 if v:
#                     covered_targets.add(t)

#     return [train_data[i] for i in selected[:k]]

# def format_sample_json(sample):
#     """Format a sample's output as JSON string."""
#     return json.dumps({
...
# def build_instruction_with_examples(examples):
#     examples_section = ""
...
# Input: {ex["text"]}
# Output:
# {format_sample_json(ex)}
# """
#     instruction = INSTRUCTION.format(examples_section=examples_section)  # fix indent + kwarg
...
# def create_conversation(sample, train_data, current_idx, target_freqs, k=3):
#     """Create a conversation with stratified diverse few-shot examples."""
#     examples = select_examples_stratified_diverse(train_data, current_idx, target_freqs, k=k)
#     instruction = build_instruction_with_examples(examples)

#     return {
#         "messages": [
#             {"role": "system", "content": instruction},
#             {"role": "user", "content": sample["text"]},
#             {"role": "assistant", "content": format_sample_json(sample)}
#         ]
#     }


# def process_all_training_data(train_data, k=3):
#     """Process entire training set."""
#     target_freqs = compute_target_frequencies(train_data)

#     conversations = []
#     for idx, sample in enumerate(train_data):
#         conv = create_conversation(sample, train_data, idx, target_freqs, k=k)
...
# train_data = [json.loads(line) for line in open("train_aggregated.jsonl")]
# val_data = [json.loads(line) for line in open("val_aggregated.jsonl")]
...
# train_convos = []
# for idx, sample in enumerate(train_data):
#     train_convos.append(create_conversation(sample, train_data, idx, target_freqs, k=3))

# # For validation, use train examples (no current_idx exclusion needed)
# val_convos = []
# for sample in val_data:
#     examples = select_examples_stratified_diverse(train_data, current_idx=-1, target_freqs=target_freqs, k=3)
#     instruction = build_instruction_with_examples(examples)
#     val_convos.append({
#         "messages": [
#             {"role": "system", "content": instruction},
...
# # Save
# with open("train_messages.jsonl", "w") as f:
#     for c in train_convos:
#         f.write(json.dumps(c) + "\n")

# with open("val_messages.jsonl", "w") as f:
#     for c in val_convos:
#         f.write(json.dumps(c) + "\n")
...
args = SFTConfig(
    output_dir="llama-1B-ctrl-alt-del-h8-r32-epochs2-alpha16-3shot-pure",         # directory to save and repository id
    max_length=2048,                         # max sequence length for model and packing of the dataset. None this
...
# %tensorboard --logdir llama-1B-ctrl-alt-del-h8-r32-epochs2-alpha16-3shot-pure
...
# !zip -r "/content/drive/MyDrive/colab_exports/llama-1B-ctrl-alt-del-h8-r32-epochs2-alpha16-3shot-pure.zip" "llama-1B-ctrl-alt-del-h8-r32-epochs2-alpha16-3shot-pure"
...
def merge_lora_model():
    ...
    model = PeftModel.from_pretrained(
        base_model,
        "/content/llama-1B-ctrl-alt-del-h8-r32-epochs2-alpha16-3shot-pure"  # Your Llama LoRA checkpoint
    )
